...
describe('Basic user flow for Website', () => {
  ...
  // Manually open the browser cleanly before tests run
  beforeAll(async () => {
  ...
  // Clean up and close the browser after all tests finish
  afterAll(async () => {
    if (browser) {
      await browser.close();
    }
  ...
  it('Make sure <product-item> elements are populated', async () => {
    ...
    for (let i = 0; i < prodItemsData.length; i++) {
      ...
      if (!value || !value.title || value.title.length == 0) { allArePopulated = false; }
      if (!value || !value.price || value.price.toString().length == 0) { allArePopulated = false; }
      if (!value || !value.image || value.image.length == 0) { allArePopulated = false; }
    }
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    // Note: The first item was already clicked in STEP 2, so we start looping from index 1
